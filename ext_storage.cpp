...
void InitializeEEProm(Print& log);
...
#define DisableInterrupts()       cli()
                                                                                //
void no_int_ext_storage_init(Print& oled) {
    oled.println(__DATE__);
    oled.print(F("Init EEPROM..."));
    ...
    if (getResetFlag != MEMORY_INITIALIZED_FLAG) {                                // if memory has never been initialized, initialize it.
        ...
        oled.println(F(" Done"));
                                                                                    //
        oled.print(F("EER..."));
        InitializeEEProm(oled);                                                         // sets all of memory = INITIAL_MEMORY_STATE_BYTE, 0xFF/255/0b11111111
        oled.println(F(" Done"));
                                                                                    //
        oled.print(F("Set Init Flag..."));
        writeResetFlag(MEMORY_INITIALIZED_FLAG);                                    // setting the last byte in external EEprom to 0x01 signals that all other 
        oled.println(F("Done"));
    } else {
        oled.println(F("EEPROM is Ok"));
    }
//
//     // if (getResetFlag != MEMORY_INITIALIZED_FLAG) {                                // if memory has never been initialized, initialize it.
...
void InitializeEEProm(Print& log) {                                                   
    ...
    byte init_buf[EEPROM_BYTES_PER_PAGE];
    memset(init_buf, INITIAL_MEMORY_STATE_BYTE, EEPROM_BYTES_PER_PAGE);

    for (uint16_t pageAddress = MIN_AVAIL_ADDR; pageAddress <= MAX_AVAIL_ADDR; pageAddress += EEPROM_BYTES_PER_PAGE) {
        eep.put(pageAddress, init_buf);
    }
}
